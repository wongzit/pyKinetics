...
def firstopen():
    ...
    if len(waveLength) == 1:
        ...
        lifetimeBtn = Button(figWin, text = "Exponential fitting", command = lambda: mono_exp_decay(startTime, endTime, waveLength[0])).grid()
    ...
    if len(waveLength) > 1:
        def showDecay2():
            ...
            lifetimeBtn2 = Button(figWin3, text = "Exponential fitting", command = lambda: mono_exp_decay(startTime, endTime, wavelen.get())).grid()
        
        tasFrame = LabelFrame(figWin, padx = 20, pady = 10)
        tasFrame.grid(padx = 10, pady = 10, sticky = W + E)
        wavelen = IntVar()
